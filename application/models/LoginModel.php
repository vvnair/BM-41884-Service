...
class LoginModel extends CI_Model {
	...
	public function userAuth($data)
	{
		...
		$result = $this->db->select('vchr_image, vchr_fname, vchr_lname, vchr_username')->from('tbl_login_details')->join('tbl_user_details', 'tbl_user_details.fk_int_lid = tbl_login_details.pk_int_lid')->where($data)->get(); //before using queries set database values and enable array('database') in autoload.php $this->db->join('comments', 'comments.id = blogs.id');
		...
		if($result->num_rows()==1)
		{
			...
			$arrayresult = array('Msg' =>'Success' ,'ResponseCode' => '200', 'Image' => $img, 'Firstname' => $fname, 'lastname' => $lname, 'Username' => $username );
		}
		else 
		{
